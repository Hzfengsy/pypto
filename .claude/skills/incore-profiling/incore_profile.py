...
# Vendored copy of PTOAS's `generate_testcase.py`, shipped next to this script
# so a PTOAS source checkout is not required. See `vendor/`.
VENDORED_GENERATE_TESTCASE = SCRIPT_DIR / "vendor" / "generate_testcase.py"


def resolve_generate_testcase(ptoas_root: Path | None) -> Path:
    """Locate the `generate_testcase.py` test-harness generator.

    Prefers an explicit `--ptoas-root` (a full PTOAS source checkout) when
    given; otherwise falls back to the vendored copy shipped alongside this
    script, so cloning the PTOAS source repo is not required.
    """
    if ptoas_root is not None:
        return ptoas_root / "test/npu_validation/scripts/generate_testcase.py"
    return VENDORED_GENERATE_TESTCASE
    ...
    try:
        ...
        if artifacts["trace_json"] and artifacts["visualize_data_bin"]:
            ...
            log(
                ...
                f"instr_csv={artifacts['instr_csv_count']} (from collect)"
            )
            return finish("exported", "collect produced traces directly")
...
def parse_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    tool_group.add_argument(
        "--ptoas-root",
        default=None,
        help="Optional PTOAS source checkout; when omitted, the vendored generate_testcase.py is used",
    )
    tool_group.add_argument(
