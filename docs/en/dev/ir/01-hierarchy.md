...
| `args_` vs callee `params_` | Identity mapping, full coverage: `args_.size() == params_.size()` | Identity mapping, **bounded** coverage: `args_.size() <= params_.size()`. Caller-supplied args come first (any direction — In, InOut, **and** caller-allocated Out); any uncovered callee params in between must be declared `Out` and are runtime-allocated (orchestration codegen synths an `add_output` for each); the trailing `CommCtxType` params appended by `MaterializeDistTensorCtx` are carried in `args_` in full, so `args_` is a plain *prefix* of `params_` only before that pass runs. Canonical statement: `Submit::args_` in `include/pypto/ir/expr.h` |
