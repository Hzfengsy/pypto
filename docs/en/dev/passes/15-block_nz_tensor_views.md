...
| `tensor.slice` of NZ (e.g. `w[b]` in an orchestration body) | rejected — pass the whole `[B, R, C]` tensor and select plane `b` in the kernel with `pl.load(w, [b, 0, 0], [1, R, C], target_memory=pl.Mem.Mat)` |
