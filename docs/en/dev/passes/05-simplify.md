...
2. **`SimplifyMutator` traversal** — extends `arith::IRMutatorWithAnalyzer`. The analyzer carries a constraint stack (loop-var bounds, if-branch conditions, scalar bindings). Folding happens at the leaves rather than only at top-level expressions because the analyzer's top-level `Simplify` does not recurse into non-arithmetic containers (`Call`, `MakeTuple`):
   ...
   - `WhileStmt`: same as `ForStmt` minus the bounds — rebuild `iter_args_` before the condition and body, then `return_vars_` after the body, with the same scoped scalar unbinding. Rebuilding `iter_args_` first is required, not cosmetic: an `IterArg` *use* is the same node as its declaration and carries `initValue_`, so the base `IRMutator` mints a fresh `IterArg` at the first use whose init the analyzer rewrote. Seeding `var_remap_` from the header makes every reference resolve to one node; skipping it leaves the header on the stale `IterArg` while all body uses point at an undefined clone (a `UseAfterDef` failure).
