...
| **FixpipeEpilogueValid** | FixpipeEpilogueValid | Every `tile.assemble` carrying a FIXPIPE epilogue (`pre_quant` / `pre_relu`) must be an **Acc -> Mat** writeback that actually uses the fix-pipe. Same-dtype assembles use a plain on-chip move and reject either pre-op. `pre_relu` alone may ride a supported unscaled narrowing. A scaled writeback is checked through `BackendHandler::SupportsFixpipePreQuant`; A2/A3 and A5 currently allow the PTOAS 0.65-validated `INT32 Acc -> FP16 Mat` (`DEQF16`) form, while unsupported source/destination pairs are rejected before pto-isa can silently drop the scale. Produced by `InferTileMemorySpace` and skipped when the backend or memory spaces are unresolved. |
