...
| -------- | -------- | -------- |
| **Structural** | TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, PipelineLoopValid, ArrayNotEscaped, ManualDepsOnSubmitOnly, AtomicAddDtypeValid, CubeTileFractalValid | Always true. Verified before/after each pass by `VerificationInstrument`; the subset shared with `GetVerifiedProperties()` is also checked at pipeline start. Normally never in PassProperties — `CubeTileFractalValid` is the one exception, see below. |
| **Pipeline** | SSAForm, NoNestedCalls, HasMemRefs, ... | Produced/invalidated by passes. Verified per pass-declared contracts. |

`GetStructuralProperties()` returns `{TypeChecked, BreakContinueValid, NoRedundantBlocks, UseAfterDef, OutParamNotShadowed, NoNestedInCore, InOutUseValid, PipelineLoopValid, ArrayNotEscaped, ManualDepsOnSubmitOnly, AtomicAddDtypeValid, CubeTileFractalValid}`. These are verified **before/after each pass** by `VerificationInstrument`. At **pipeline start**, `PassPipeline::Run()` verifies only the lightweight subset shared with `GetVerifiedProperties()` (`GetStructuralProperties().Intersection(GetVerifiedProperties())`) — so e.g. `ArrayNotEscaped` is checked before/after each pass but not at pipeline start. Since no pass declares them in `required`/`produced`/`invalidated`, `VerificationInstrument` unions them with the pass's declared properties to ensure no pass breaks these fundamental invariants.

**`CubeTileFractalValid` is the one structural property a pass does declare.**
`ConvertTensorToTileOps` and `AutoTileMatmulL0` both list it in `invalidated`
*and* `produced`. A structural property is normally an invariant no pass may
break, but these two *synthesize* the cube operands whose physical shapes the
property constrains: at pipeline input a tensor-level program has no
`tile.matmul` at all, so the check passes vacuously. Declaring it re-runs the
check against the tiles each pass actually created, rather than against the
empty input it already passed. The union above still applies, so every other
pass is held to the invariant as usual.
...
| **CubeTileFractalValid** | CubeTileFractalValid | Every cube matmul-family operand declares a **physical** tile whose row count is a multiple of the NZ fractal box height `M0 = 16`. Covers `tile.matmul` / `_acc` / `_bias`, the `tile.gemv` family and the `tile.batch_matmul` family, on the Left and Acc roles (the Right role's outer index is K, not M). pto-isa keeps two independent notions of size here and only one is constrained: the **logical** extent is what the instruction computes over, read from the operands' valid region (`docs/isa/tile/matrix-and-matrix-vector.md`: "For M = a.GetValidRow()...") and bounded on A2A3 only at `[1, 4095]` — `m = 1` is a documented case, which is why `pto.mad` carries a `disable_gemv` clause at all. The **physical** extent is how many bytes the tile occupies, and the cube addresses L0A/L0B/L0C in whole fractal boxes (`docs/isa/cube/nz-fractal-layout.md`, machine constant `FRACTAL_NZ_ROW = 16`). A narrower logical extent inside a full-size tile needs no help — "compact mode is automatic in matmul when valid region < physical tile size" — but a sub-fractal *physical* tile cannot hold one box and nothing downstream repairs it: `AutoTileMatmulL0` declined to tile it and reported a perf hint, ptoas accepted the shape, and the emitted `TMATMUL` named a `rows=1` L0A/L0C tile. The same computation spelled `pl.tile.gemv` allocates a 16-row accumulator (`BuildGemvResultType`), which is the shape the cube actually wants. GEMV's own one-row **left** operand is exempt: that is the required A-vector organization, not a sub-fractal tile. **Scope**: the M axis only. The manual gives `K0 = N0 = C0 / E` for the other two, which agrees with the 512-byte `CUBE_BLOCK_SIZE` box for a 2-byte dtype but not for `int8` or `f32`, so those axes are left unenforced pending confirmation from the ISA owners rather than guessed at. Listed in **`GetStructuralProperties()`**, so `PassPipeline` verifies it at `pipeline_input` for a tile-level program and the error carries the user's own `Span`; additionally **produced by** (and invalidated by) `ConvertTensorToTileOps`, which is where a tensor-level `pl.matmul` first acquires cube operands, and by `AutoTileMatmulL0`, which re-derives every operand shape. Skipped when the backend is unconfigured. **Fix**: declare the tile at a fractal multiple and carry the true extent in `valid_shape` — a load moves only the valid extent, so the padding costs no DMA. For a single-row left operand, `pl.tile.gemv` is the dedicated cube instruction and avoids the padding entirely. |
