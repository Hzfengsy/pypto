...
| **AccToGmStoreValid** | AccToGmStoreValid | Every `tile.store` whose source tile is **Acc-resident** targets a GM tensor whose dtype the cube fix-pipe can narrow into: `INT32/FP32/FP16/BF16` on both Ascend910B and Ascend950 (`BackendHandler::SupportsAccToGmDtype`, mirroring pto-isa's non-quant `CheckAcc2gm` whitelist and ptoas' `pto.tstore` "acc tstore dst element type" rule; the hook stays per-backend because the two sets are independent facts about each pinned target). An `INT8`/`INT16` destination is rejected. The whitelist is a set of *destinations*, not of conversions, so passing it is necessary and not sufficient: the store must also be one the fix-pipe can reach **from this accumulator**. The unscaled writeback performs exactly one conversion, `FP32 -> FP16` / `FP32 -> BF16` (`ir::CubeWritebackSupportsDataType`, mirroring pto-isa's `GetCastPreQuantMode`, which agrees on a2a3 and a5), so an `INT32` accumulator may only leave as `INT32`. An `INT8 x INT8` matmul stored straight into an `FP32` tensor passes the whitelist and is still a dequantization with no scale: ptoas accepts it, then ccec fails inside pto-isa's `TStoreAcc` ("the 2nd parameter maybe need a type `__cc__ float *`"), or where the shape lets it compile the kernel returns raw accumulator bits reinterpreted as float. **Produced by** `InferTileMemorySpace` and listed in `GetVerifiedProperties()`, so `PassPipeline` auto-verifies it right after that pass. That placement is load-bearing: legality is a property of the tile's **memory space**, not of the user-visible dtypes, so it cannot be decided earlier — the identical DSL program is legal when its matmul result routes through Vec (an explicit `pl.cast` narrows in the vector unit) and illegal when it stays in Acc. Without this check the program reaches ptoas, which rejects it at `pto.tstore` verification against a line in a generated `.pto`; one such store also tends to trip a second, unrelated-looking op (an int8 zero-init lowers to a `pto.texpands` with the same illegal dtype), so the late diagnostic names two symptoms and never the cause. A store carrying a FIXPIPE epilogue takes a different route through this same property: `pre_quant` replaces *both* rules above with one query against the scale-bearing table (`BackendHandler::SupportsFixpipePreQuant`), which is how a requantizing store reaches the very `INT8` the unscaled whitelist excludes, and an epilogue on a non-Acc source is rejected outright rather than left to ptoas. The Acc->Mat half of that contract is `FixpipeEpilogueValid`. Skipped when the backend is unconfigured (nothing to verify against) or the tile's memory space is unresolved. **Fix**: narrow through the vector unit — `pl.cast` the matmul result to the destination dtype and store that — or accumulate into an `INT32`/`FP32` tensor and convert afterwards. |
| **FixpipeEpilogueValid** | FixpipeEpilogueValid | Every `tile.assemble` carrying a FIXPIPE epilogue -- `pre_quant` (an FP32 scale the accumulator is multiplied by on the way out) or `pre_relu` (an activation applied to the **accumulator**, *ahead* of that multiply -- pto-isa spells it `ReluPreMode` -- with the destination clamp last) -- is an **Acc -> Mat** writeback that actually goes through the fix-pipe. Two rules, one per pre-op. **Both** need the writeback to *convert*: Acc -> Mat has two lowerings, and a same-dtype assemble is an MTE1 `pto.subview` + `pto.tmov` with no fix-pipe to configure (`ir::CubeMatWritebackUsesFixpipe`, the one predicate the emitter shares, so the two cannot disagree about which lowering a program gets). That alone suffices for **`pre_relu`**, which rides the unscaled `FP32 -> FP16/BF16` narrowing and is the half of this feature that works on Acc -> Mat today. **`pre_quant` on Acc -> Mat is withheld on every backend** (`BackendHandler::SupportsFixpipePreQuant` answers `false` for `FixpipeDest::kMat`), because ptoas cannot emit it ([PTOAS#1570](https://github.com/hw-native-sys/PTOAS/issues/1570)): it assembles the scale onto `pto.tinsert` -- the form round-trips through `--emit-pto-ir` -- but every scalar operand it emits is `int64_t`, and pto-isa's two `TINSERT` wrappers in the arch-independent `pto/common/pto_instr.hpp` differ only in their parameter types (`(Dst&, Src&, uint16_t row, uint16_t col, WaitEvents&...)` versus `(Dst&, Src&, uint64_t, uint16_t row, uint16_t col, WaitEvents&...)`). For `TINSERT<Mat, Acc, relu>(dst, src, scale_i64, row_i64, col_i64)` the unscaled one is the better match -- `col` binds to the event pack by identity where the scaled one needs a conversion -- so the scale lands in `row`, truncated to `0`. On a2a3 the resulting cast path `static_assert`s inside `CheckTMovAccToMat`; a5 has no such guard, so there it would be a silent wrong answer. The diagnostic names the toolchain rather than the dtypes, since no dtype choice avoids it. The dtype table behind `SupportsFixpipePreQuant` is retained and still governs the Acc -> GM half (`AccToGmStoreValid`), where the scale is what makes `INT32 -> FP16` (`DEQF16`), `INT32 -> INT8` (`REQ8`) and `FP32 -> INT8` (`QF322B8_PRE`) reachable from a cube accumulator; `INT16` is excluded everywhere even though pto-isa lists it, because its `SHIFTS322S16` payload is a shift count, not a scale. Re-enabling Acc -> Mat is one line per handler once ptoas emits the call unambiguously; `tests/ut/codegen/test_pto_codegen_ops.py::TestFixpipeEpilogueCodegen` keeps the emission pinned meanwhile (with verification off). An epilogue on any move that is not Acc -> Mat is rejected outright: there is no fix-pipe to configure. **Produced by** `InferTileMemorySpace` and listed in `GetVerifiedProperties()`, for the same reason as `AccToGmStoreValid` -- legality is a property of the tile's memory space, so it cannot be decided earlier. **This one is an error rather than a perf hint because nothing downstream catches it**: ptoas verifies the quantized `pto.tinsert` dtype pair on a2a3 only, and pto-isa answers an unsupported pair from `GetScalarPreQuantMode` with `QuantMode_t::NoQuant`, which silently *drops the scale*. Skipped when the backend is unconfigured or the tile's memory spaces are unresolved. **Fix**: store the scaled accumulator straight to GM instead -- `pl.tile.store(acc, ..., pre_quant=s)`, which is device-verified -- or scale and activate in the vector unit, `pl.maximum(pl.mul(pl.cast(result, ...), scale), 0.0)`, and move that to Mat. |
...
| `GetVerifiedProperties()` | `{SSAForm, TypeChecked, InlineFunctionsEliminated, MixedKernelExpanded, AllocatedMemoryAddr, BreakContinueValid, NoRedundantBlocks, InOutUseValid, CallDirectionsResolved, ManualDepsOnSubmitOnly, ReturnParamsExplicit, AivSplitValid, AivSplitLoweredValid, TileMemoryInferred, TileOps2D, HardSyncallOccupancyValid, IterArgCarryClassified, RuntimeScopesMaterialized, DistTensorCtxMaterialized, GraphBoundaryLegalized, AccToGmStoreValid, FixpipeEpilogueValid, AccCompactValid, AtomicAddDtypeValid, AccStorePhaseValid}` | Lightweight set for `PassPipeline` auto-verify |
