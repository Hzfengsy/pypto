...
| PTOAS | 标准 wheel 启动器使用其解释器实际选中的包元数据及原生编译器 Build-ID；独立 ELF 使用 Build-ID；其他启动器使用保留开发后缀的完整 `--version` 输出。 |
| 设备及编排工具链 | 选中的编译器路径/版本、CANN 安装构建版本及链接器 Build-ID。 |
...
此策略信任发布的原生构建/版本标识，不扫描系统头文件、Python 标准库、传递动态库
依赖或 runtime C++ 未提交修改。若修改这些输入而没有更新版本标识，应设置新的
`PYPTO_CACHE_EPOCH` 或清空缓存。进程内安装文件应保持不变，替换后重启进程。
