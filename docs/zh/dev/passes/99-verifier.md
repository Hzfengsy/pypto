...
| **CubeTileFractalValid** | CubeTileFractalValid | 每个 cube 矩阵乘算子的操作数所声明的**物理** tile，其行数必须是 NZ 分形块高度 `M0 = 16` 的整数倍。覆盖 `tile.matmul` / `_acc` / `_bias`、`tile.gemv` 系列与 `tile.batch_matmul` 系列的 Left 与 Acc 角色（Right 角色的外层维度是 K 而非 M）。pto-isa 在此保有两个彼此独立的尺寸概念，且只约束其中之一：**逻辑**尺寸是指令实际参与计算的范围，取自操作数的 valid 区域（`docs/isa/tile/matrix-and-matrix-vector.md`："For M = a.GetValidRow()..."），在 A2A3 上仅受 `[1, 4095]` 限制——`m = 1` 是文档明确支持的情形，这也正是 `pto.mad` 带有 `disable_gemv` 子句的原因。**物理**尺寸则是该 tile 占用的字节数，cube 以完整分形块为单位寻址 L0A/L0B/L0C（`docs/isa/cube/nz-fractal-layout.md`，机器常量 `FRACTAL_NZ_ROW = 16`）。整块 tile 内部的较窄逻辑区域无需额外处理——"compact mode is automatic in matmul when valid region < physical tile size"——但小于一个分形块的**物理** tile 连一个块都装不下，且下游无人修复：`AutoTileMatmulL0` 放弃切分并只报了一条 perf hint，ptoas 接受了该 shape，最终发射的 `TMATMUL` 指向一个 `rows=1` 的 L0A/L0C tile。同一计算若写成 `pl.tile.gemv`，则会分配 16 行累加器（`BuildGemvResultType`），那才是 cube 需要的形状。GEMV 自身单行的 **left** 操作数属于豁免情形：那是规定的 A 向量组织方式，并非小于分形块的 tile。**范围**：仅 M 轴。手册对另外两轴给出 `K0 = N0 = C0 / E`，该式在 2 字节 dtype 下与 512 字节的 `CUBE_BLOCK_SIZE` 块一致，但对 `int8` 与 `f32` 并不一致，因此在 ISA 负责人确认规则之前，这两个轴暂不校验，而不是凭猜测设限。列入 **`GetStructuralProperties()`**，因此对 tile 级程序，`PassPipeline` 在 `pipeline_input` 阶段验证，错误携带用户自己的 `Span`；此外由 `ConvertTensorToTileOps`（tensor 级 `pl.matmul` 首次获得 cube 操作数之处）与 `AutoTileMatmulL0`（重新推导全部操作数形状）**产生**并失效。后端未配置时跳过。**修复方式**：将 tile 声明为分形整数倍，并用 `valid_shape` 承载真实范围——load 只搬运 valid 范围，因此这段 padding 不产生额外 DMA。若 left 操作数只有一行，`pl.tile.gemv` 是专用的 cube 指令，可完全免去 padding。 |
