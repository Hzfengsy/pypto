...
| **AccToGmStoreValid** | AccToGmStoreValid | 每个源 tile 位于 **Acc** 的 `tile.store`，其目标 GM tensor 的 dtype 必须是 cube fix-pipe 能够收窄到的类型：Ascend910B 与 Ascend950 均为 `INT32/FP32/FP16/BF16`（`BackendHandler::SupportsAccToGmDtype`，与 pto-isa 非量化分支的 `CheckAcc2gm` 白名单以及 ptoas `pto.tstore` 的 "acc tstore dst element type" 规则一致；该钩子仍按后端区分，因为两个集合是各自 pin 目标的独立事实）。`INT8`/`INT16` 目标会被拒绝。该白名单是一组**目标类型**而非一组转换，因此通过它是必要而非充分条件：这次 store 还必须是 fix-pipe **能从当前累加器出发**完成的。非量化写回只做一种转换，`FP32 -> FP16` / `FP32 -> BF16`（`ir::CubeWritebackSupportsDataType`，与 pto-isa 的 `GetCastPreQuantMode` 一致，a2a3 与 a5 相同），所以 `INT32` 累加器只能以 `INT32` 离开。`INT8 x INT8` matmul 直接存入 `FP32` tensor 能通过白名单，但它依然是一次没有 scale 的反量化：ptoas 会接受它，随后 ccec 在 pto-isa 的 `TStoreAcc` 中报错（"the 2nd parameter maybe need a type `__cc__ float *`"）；若形状恰好能编译通过，kernel 会把原始累加器比特按 float 重新解释后写出。**由** `InferTileMemorySpace` **产生**并列入 `GetVerifiedProperties()`，因此 `PassPipeline` 在该 pass 之后立即自动验证。这个位置是关键的：合法性取决于 tile 的**内存空间**，而非用户可见的 dtype，因此无法更早判定——同一段 DSL 程序，当 matmul 结果经由 Vec（显式 `pl.cast` 在向量单元中收窄）时合法，留在 Acc 时则非法。缺少此检查时程序会一路走到 ptoas，由后者在 `pto.tstore` 校验阶段针对生成的 `.pto` 中某一行报错；而且这样一个 store 往往还会连带触发另一个看似无关的算子（int8 零初始化会下降为同样非法 dtype 的 `pto.texpands`），于是这个迟到的诊断只报出两个症状，从不指出根因。携带 FIXPIPE epilogue 的 store 在同一个 property 下走另一条路径：`pre_quant` 会把上面**两条**规则一并替换为对带 scale 表（`BackendHandler::SupportsFixpipePreQuant`）的一次查询——重量化 store 正是这样抵达非量化白名单所排除的 `INT8`；而非 Acc 源上的 epilogue 会被直接拒绝，不再留给 ptoas。该契约的 Acc->Mat 那一半是 `FixpipeEpilogueValid`。当后端未配置（无可校验的依据）或 tile 内存空间尚未解析时跳过。**修复方式**：先经向量单元收窄——用 `pl.cast` 将 matmul 结果转为目标 dtype 后再存储——或者累加到 `INT32`/`FP32` tensor 后再转换。 |
| **FixpipeEpilogueValid** | FixpipeEpilogueValid | 每个携带 FIXPIPE epilogue 的 `tile.assemble` ——`pre_quant`（排空累加器时乘上的 FP32 scale）或 `pre_relu`（施加在**累加器**上、位于该乘法**之前**的激活，pto-isa 称之为 `ReluPreMode`，目标 clamp 排在最后）——都必须是一次**真正走 fix-pipe** 的 **Acc -> Mat** 写回。两个前置操作各有一条规则。**两者都**要求这次写回发生**类型转换**：Acc -> Mat 有两种下降路径，同 dtype 的 assemble 是 MTE1 的 `pto.subview` + `pto.tmov`，路径上根本没有 fix-pipe 可配置（`ir::CubeMatWritebackUsesFixpipe`，emitter 与本 verifier 共用的同一判据，因此两者不会对「某段程序走哪条下降」产生分歧）。对 **`pre_relu`** 而言这一条就够了——它搭的是非量化的 `FP32 -> FP16/BF16` 收窄，也是这个特性在 Acc -> Mat 上目前唯一可用的那一半。**`pre_quant` 在 Acc -> Mat 上于所有后端都被暂时关闭**（`BackendHandler::SupportsFixpipePreQuant` 对 `FixpipeDest::kMat` 一律返回 `false`），原因是 ptoas 发不出这条指令（[PTOAS#1570](https://github.com/hw-native-sys/PTOAS/issues/1570)）：它能把 scale 汇编到 `pto.tinsert` 上（该写法可经 `--emit-pto-ir` 往返），但它生成的每个标量操作数都是 `int64_t`，而 pto-isa 在与架构无关的 `pto/common/pto_instr.hpp` 中的两个 `TINSERT` 包装仅在形参类型上不同：`(Dst&, Src&, uint16_t row, uint16_t col, WaitEvents&...)` 与 `(Dst&, Src&, uint64_t, uint16_t row, uint16_t col, WaitEvents&...)`。对 `TINSERT<Mat, Acc, relu>(dst, src, scale_i64, row_i64, col_i64)` 这次调用，不带 scale 的那个重载匹配更优——`col` 以恒等方式绑定到事件包，而带 scale 的那个在此处需要一次整型转换——于是 scale 落进了 `row`，被截断为 `0`。在 a2a3 上，随之走到的 cast 路径会在 `CheckTMovAccToMat` 里 `static_assert` 失败；a5 没有这道防线，在那里就是一个静默的错误结果。因此诊断信息指向工具链而不是 dtype：换任何 dtype 都绕不开。`SupportsFixpipePreQuant` 背后的 dtype 表保留下来，仍然管辖 Acc -> GM 那一半（`AccToGmStoreValid`）——正因为带了 scale，`INT32 -> FP16`（`DEQF16`）、`INT32 -> INT8`（`REQ8`）与 `FP32 -> INT8`（`QF322B8_PRE`）才能从 cube 累加器直接抵达；`INT16` 在所有后端都被排除，尽管 pto-isa 列出了它——其 `SHIFTS322S16` 的负载是**移位位数**而不是 scale。等 ptoas 能无歧义地发出这次调用，每个 handler 改一行即可重新开启；在此期间由 `tests/ut/codegen/test_pto_codegen_ops.py::TestFixpipeEpilogueCodegen` 钉住发射形态（关闭校验运行）。任何非 Acc -> Mat 的搬运上出现 epilogue 都直接拒绝：那里根本没有 fix-pipe 可配置。**由** `InferTileMemorySpace` **产生**并列入 `GetVerifiedProperties()`，理由与 `AccToGmStoreValid` 相同——合法性取决于 tile 的内存空间，无法更早判定。**这里是 error 而非 perf hint，因为下游没有任何兜底**：ptoas 只在 a2a3 上校验量化 `pto.tinsert` 的 dtype 组合，而 pto-isa 的 `GetScalarPreQuantMode` 对不支持的组合返回 `QuantMode_t::NoQuant`，即**静默丢掉 scale**。当后端未配置或 tile 内存空间尚未解析时跳过。**修复方式**：改为把带 scale 的累加器直接存到 GM——`pl.tile.store(acc, ..., pre_quant=s)`，这条路径已在真机上验证过——或者改在向量单元里缩放与激活（`pl.maximum(pl.mul(pl.cast(result, ...), scale), 0.0)`）再把结果搬到 Mat。 |
...
不能证明互不重叠。重叠的 `tile.move` 操作数会被拒绝，包括完全相同的自复制，
因为已分配地址的 TMOV 要求源和目标地址不同。PTOAS 使用符号存储身份。无法证明同时存活
...
| `GetVerifiedProperties()` | `{SSAForm, TypeChecked, InlineFunctionsEliminated, MixedKernelExpanded, AllocatedMemoryAddr, BreakContinueValid, NoRedundantBlocks, InOutUseValid, CallDirectionsResolved, ManualDepsOnSubmitOnly, ReturnParamsExplicit, AivSplitValid, AivSplitLoweredValid, TileMemoryInferred, TileOps2D, HardSyncallOccupancyValid, IterArgCarryClassified, RuntimeScopesMaterialized, DistTensorCtxMaterialized, GraphBoundaryLegalized, AccToGmStoreValid, FixpipeEpilogueValid, AccCompactValid, AtomicAddDtypeValid, AccStorePhaseValid}` | `PassPipeline` 自动验证的轻量级属性集 |
