...
| **FixpipeEpilogueValid** | FixpipeEpilogueValid | 每个携带 FIXPIPE epilogue（`pre_quant` / `pre_relu`）的 `tile.assemble` 都必须是一次真正使用 fix-pipe 的 **Acc -> Mat** 写回。同 dtype assemble 走普通片上搬运，不能携带任何 pre-op；单独的 `pre_relu` 可以搭载在受支持的无 scale 收窄写回上。带 scale 的写回通过 `BackendHandler::SupportsFixpipePreQuant` 校验；A2/A3 与 A5 当前开放经 PTOAS 0.65 验证的 `INT32 Acc -> FP16 Mat`（`DEQF16`）形式，其他源/目标 dtype 组合会在 pto-isa 静默丢弃 scale 之前被拒绝。该属性由 `InferTileMemorySpace` 产生；后端未配置或内存空间未解析时跳过。 |
